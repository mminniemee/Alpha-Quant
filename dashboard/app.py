...
# This must remain at the very top of the file before any other imports.
# It forces the Linux server to recognize the parent directory containing 'src'.
# ==============================================================================
...
# Now that the path is fixed, we can safely import from our src folder
...
# Set up standard seeded positions matching our dynamic Nifty 500 choices
...
# Automatically source top 20 performing stocks from Nifty 500 CSV
WATCHLIST = get_top_fundamental_symbols(limit=20)
...
with st.sidebar:
    ...
    if st.button("🔄 Run Full Nifty 500 Scan"):
        progress_text = "Scanning Nifty 500..."
...
with tab_charts:
    ...
    if df_chart is not None and len(df_chart) >= 50:
        # Calculate daily indicators on live data so sma20 is populated
        df_chart = apply_daily_indicators(df_chart)
    else:
        # Build simulated candles for offline/weekend demonstration
        np.random.seed(42)
        ...
        if st.button("🚀 Execute Manual Order"):
            try:
                ...
                if man_action == "BUY":
                    curr_port[man_sym] = {
                        "qty": man_qty,
                        "entry_price": man_price,
                        "setup_low": None,
                        "highest_close": man_price,
                        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    }
...
with tab_watchlist:
    ...
    st.write("Dynamically manage the underlying Nifty 500 watchlist database directly from the browser.")
            ...
            if not new_sym:
                ...
                try:
                    ...
                    new_row = {
                        ...
                        "pbt_latest": new_pbt_latest,
                        "pbt_prev": new_pbt_prev,
                        "growth_pct": round(calc_growth, 1)
                    }
...
with tab_performance:
    ...
    with graph_col1:
        ...
        fig_curve.add_trace(go.Scatter(
            x=graph_df.index, y=graph_df['AlphaQuant Capital Growth'],
            mode='lines', name='AlphaQuant Capital',
            line=dict(color='#10b981', width=3)
        ))
        fig_curve.add_trace(go.Scatter(
            x=graph_df.index, y=graph_df['Nifty 50 Index Benchmark'],
...
with tab_audit_log:
    ...
    if history_data:
        ...
        for item in history_data:
            ...
            audit_rows.append({
                "Timestamp (IST)": item['timestamp'], "Asset": item['symbol'], "Action Taken": emoji,
